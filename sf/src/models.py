...
def boltzmannFactorQ(ensemble_energies: torch.tensor):
    ...
    ensemble_energies = 1/Q * torch.sum(torch.mul(torch.exp(-RT*ensemble_energies).reshape(-1,1), ensemble_energies))
    return ensemble_energies
...
class AffiNETy_graphSage_boltzmann_avg_Q(nn.Module):
    ...
    def forward(self, data, device):
        ...
        pl_e_avg = boltzmannFactorRescaling(pl_es)
        p_e_avg = boltzmannFactorRescaling(p_es)
        l_e_avg = boltzmannFactorRescaling(l_es)
...
class AffiNETy:
    def __init__(
        ...
        pl_model=GraphSAGE(
            in_channels=-1, hidden_channels=5, num_layers=3, out_channels=1, jk="max"
        ),
        p_model=GraphSAGE(
            in_channels=-1, hidden_channels=5, num_layers=3, out_channels=1, jk="max"
        ),
    ...
    @record
    def train(self, epochs=100, batch_size=2, split_percent=0.8, verbose=False, pre_trained_model=None):
        ...
        for epoch in range(epochs):
            ...
            if eval_loss < lowest_test_error:
                torch.save(self.model.state_dict(), "models/AffiNETy_mean.pt")
            # for name, param in self.model.named_parameters():
            #     if param.requires_grad:
            #         print(name, param.data)
        return
...
class AffiNETy_ViSNet:
    ...
    def train(self, epochs=100, batch_size=2, split_percent=0.8, verbose=True):
        ...
        for epoch in range(epochs):
            ...
            print(f"Epoch {epoch+1}/{epochs}, Train: {train_loss}, Eval: {eval_loss}")
            if eval_loss < lowest_test_error:
                torch.save(self.model.state_dict(), "models/AffiNETy_visnet.pt")
        return
